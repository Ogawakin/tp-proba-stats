...
def intervalle_confiance(alpha, liste):
    ...
    ecart_type = np.sqrt(variance_empirique(liste))
    t = 0
    if alpha == 0.05:
        t = 1,746
    elif alpha == 0.01:
    ...
    intervalle_gauche = moyenne - t*ecart_type/(np.sqrt(len(liste)))
    intervalle_droit = moyenne + t*ecart_type/(np.sqrt(len(liste)))
